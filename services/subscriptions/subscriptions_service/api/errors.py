...
from subscriptions_service.core.exceptions import AppError

logger = logging.getLogger(__name__)


def _error_payload(request: Request, code: str, message: str) -> dict[str, str]:
    return {
        "code": code,
        "message": message,
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def handle_validation(request: Request, _: RequestValidationError) -> JSONResponse:
        return JSONResponse(
            status_code=422,
            content=_error_payload(request, "validation_error", "Invalid request"),
        )
